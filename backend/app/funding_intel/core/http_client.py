"""Shared async HTTP client for all funding-intel providers.

Centralises connection pooling, timeouts, retries with exponential
backoff, and a uniform user-agent string. Providers should construct
this client once in ``initialize()`` and reuse it for every call.
"""
...
from .config import funding_intel_settings
...
class ProviderHTTPClient:
    ...
    async def request(
        ...
        *,
        ...
        Raises ``httpx.HTTPError`` (or subclass) if every retry fails.
        The caller is expected to catch and log via the provider's
        health record.
        """
        attempt = 0
            ...
            try:
                ...
                if response.status_code in (429, 500, 502, 503, 504):
                    if attempt <= self.max_retries:
                        ...
                        continue
                    # Final attempt failed — fall through and raise below
                    response.raise_for_status()

                if expect_json and response.headers.get("content-type", "").split(";")[0].strip() != "application/json":
                    # NIH sometimes returns JSON without a header. Best effort.
        ...
        # If we reach here every retry failed.
        if last_exc:
            raise last_exc
    ...
    def safe_get(d: Dict[str, Any], *keys: str, default: Any = None) -> Any:
        """Traverse nested dicts without raising on missing keys."""
        cur: Any = d
        for k in keys:
