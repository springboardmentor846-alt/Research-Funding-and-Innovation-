...
registry so legacy code that references the class still works.
"""
...
class USPTOProvider(BasePatentProvider):
    """Adapter for the PatentsView public patent search API."""
    ...
    def __init__(self) -> None:
        ...
        self._base_url = s.USPTO_BASE_URL
    ...
    def normalize(self, raw: Dict[str, Any]) -> Optional[NormalizedPatent]:
        ...
        patent_number = (
            ...
            or raw.get("publication_number")
            or (raw.get("patents") or [{}])[0].get("patent_number") if isinstance(raw.get("patents"), list) and raw.get("patents") else ""
        )
        patent_number = (patent_number or "").strip()
        if not patent_number:
            return None
        title = (
            raw.get("title")
            or raw.get("patent_title")
            or (raw.get("patents") or [{}])[0].get("patent_title") if isinstance(raw.get("patents"), list) and raw.get("patents") else ""
        )
        ...
        abstract = (
            raw.get("abstract")
            or raw.get("patent_abstract")
            or (raw.get("patents") or [{}])[0].get("patent_abstract") if isinstance(raw.get("patents"), list) and raw.get("patents") else None
        )
        inventors = raw.get("inventors") or raw.get("inventor_first_name")
        if isinstance(inventors, list):
            inventors = ", ".join(str(i) for i in inventors if i)
        assignee = raw.get("assignee") or raw.get("assignee_organization")
        if isinstance(assignee, list):
            assignee = ", ".join(str(a) for a in assignee if a)
        return NormalizedPatent(
            source=self.name,
            source_id=str(raw.get("source_id") or raw.get("id") or patent_number),
            ...
            classification=raw.get("classification") or raw.get("cpc"),
            classification_label=raw.get("classification_label") or raw.get("cpc_label"),
