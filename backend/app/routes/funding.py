...
def eligible_funding(
    ...
    if not profile:
        return {"message": "Research Profile not found"}
...
def search_by_source_type(
    ...
    funding = db.query(FundingOpportunity).filter(
        FundingOpportunity.source_type.ilike(f"%{source_type}%")
    ).all()
...
def search_by_agency(
    ...
    funding = db.query(FundingOpportunity).filter(
        FundingOpportunity.funding_agency.ilike(f"%{agency}%")
    ).all()
...
def funding_statistics(
    ...
    government = db.query(FundingOpportunity).filter(
        FundingOpportunity.source_type == "Government Grant"
    ).count()

    innovation = db.query(FundingOpportunity).filter(
        FundingOpportunity.source_type == "Innovation Fund"
    ).count()

    startup = db.query(FundingOpportunity).filter(
        FundingOpportunity.source_type == "Startup Accelerator"
    ).count()

    international = db.query(FundingOpportunity).filter(
